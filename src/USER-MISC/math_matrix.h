...
///
/// @note
///   If the matrices you want to analyze are ordinary square matrices,
///   it might be easier to use "PEigenCalculator" instead.  (It takes care
///   of the LambdaLanczos details for you.)
///
/// @note:
...
public:
  ...
  LambdaLanczos(std::function<void(const std::vector<T>&, std::vector<T>&)> mv_mul, int matrix_size) : LambdaLanczos(mv_mul, matrix_size, false) {}

  /// @brief  Specify the size of the matrix you will analyze.
  ///         (This equals the size of the eigenvector which will be returned.)
  int matrix_size;

  /// @brief  Are we serching for the maximum or minimum eigenvalue?
  bool find_maximum = false;

  /// @brief  Shift all the eigenvalues by "eigenvalue_offset"
  ///        (This can also be used to select the smallest eigenvalue instead of
  ///         the largest eigenvalue.  See PEigenCalculator::PrincipalEigen())
  ...
  void SetMul(std::function<void(const std::vector<T>&, std::vector<T>&)> mv_mul);
...
// --- Implementation of PrincipalEigen (a wrapper for dense matrices)
               ...
               bool find_max)
{
  ...
  if (! find_max) {
    ...
    // largest magnitude provides an upper bound for the eigenvalue magnitudes.
    // If the caller requested the minimum eigenvalue (closest to -infinity)
    // then we must shift all the eigenvalues in that direction to make so that
    // this eigenvalue has the largest magnitude (after shifting) beforehand.
    Scalar eval_upper_bound = 0.0;
    for (int i = 0; i < n; i++)
      for (int j = 0; j < n; i++)
        if (eval_upper_bound < std::abs(matrix[i][j]))
          eval_upper_bound = std::abs(matrix[i][j]);
