...
FixStyle(bond/modify,FixBondModify)

#else

#ifndef LMP_FIX_BOND_MODIFY_H
#define LMP_FIX_BOND_MODIFY_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBondModify : public Fix {
 public:
  FixBondModify(class LAMMPS *, int, char **);
  ~FixBondModify();
  int setmask();
  void init();
  void post_integrate();
 ...
 private:
  ...
  void CheckAtomConsistency();
  void CheckBondConsistency();
};
...
E: Invalid bond type in fix bond/modify command

Self-explanatory.

E: Cannot use fix bond/modify with non-molecular systems

Only systems with bonds that can be changed can be used.  Atom_style
template does not qualify.

E: Cannot yet use fix bond/modify with this improper style

This is a current restriction in LAMMPS.

E: Fix bond/modify needs ghost atoms from further away
